...
async def async_setup_entry(
    ...
    sensors = [
        ...
        DreameMowerDNDSensor(coordinator),
        DreameMowerChargingScheduleSensor(coordinator),
        DreameMowerTimezoneSensor(coordinator),
...
class DreameMowerDNDSensor(DreameMowerEntity, SensorEntity):
    """Do Not Disturb sensor for Dreame Mower."""

    def __init__(self, coordinator: DreameMowerCoordinator) -> None:
        """Initialize the DND sensor."""
        super().__init__(coordinator, "dnd_status")
        self._attr_icon = "mdi:sleep"
        self._attr_entity_category = EntityCategory.DIAGNOSTIC
        self._attr_translation_key = "dnd_status"

    @property
    def native_value(self) -> bool | None:
        """Return the DND status."""
        return self.coordinator.device_dnd_enabled

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return extra state attributes."""
        # Start with empty dict and explicitly type it
        attributes: dict[str, Any] = {}
        attributes["dnd_enabled"] = self.coordinator.device_dnd_enabled
        
        # Add time information if available
        start_time = self.coordinator.device_dnd_start_time
        end_time = self.coordinator.device_dnd_end_time
        
        if start_time is not None:
            attributes["start_time"] = self._format_time_minutes(start_time)
            
        if end_time is not None:
            attributes["end_time"] = self._format_time_minutes(end_time)
...
class DreameMowerChargingScheduleSensor(DreameMowerEntity, SensorEntity):
    """Charging schedule sensor for Dreame Mower."""

    def __init__(self, coordinator: DreameMowerCoordinator) -> None:
        """Initialize the charging schedule sensor."""
        super().__init__(coordinator, "charging_schedule")
        self._attr_icon = "mdi:battery-charging"
        self._attr_entity_category = EntityCategory.DIAGNOSTIC
        self._attr_translation_key = "charging_schedule"

    @property
    def native_value(self) -> bool | None:
        """Return the charging schedule enabled status."""
        charging_data = self.coordinator.current_charging_schedule_data
        if not charging_data:
            return None
        return charging_data.get("enabled")
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        charging_data = self.coordinator.current_charging_schedule_data
        if charging_data:
            attributes["enabled"] = charging_data.get("enabled")
            attributes["auto_recharge_percent"] = charging_data.get("auto_recharge_percent")
            attributes["resume_percent"] = charging_data.get("resume_percent")
            attributes["start_time"] = self._format_time_minutes(charging_data.get("start_minute"))
            attributes["end_time"] = self._format_time_minutes(charging_data.get("end_minute"))

        return attributes
    
    @staticmethod
    def _format_time_minutes(minutes: int | None) -> str | None:
        """Convert minutes from midnight to HH:MM format."""
        if minutes is None:
            return None
        hours = minutes // 60
...
class DreameMowerTimezoneSensor(DreameMowerEntity, SensorEntity):
    """Timezone sensor for Dreame Mower."""
    ...
    def __init__(self, coordinator: DreameMowerCoordinator) -> None:
        ...
        self._attr_entity_category = EntityCategory.DIAGNOSTIC
        self._attr_translation_key = "timezone"

    @property
    def native_value(self) -> str | None:
        """Return the device timezone."""
        return self.coordinator.device_timezone

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return extra state attributes."""
        return {
            "timezone": self.coordinator.device_timezone,
            "device_time": self._format_device_time(self.coordinator.device_time),
        }

    @staticmethod
    def _format_device_time(device_time: str | None) -> str | None:
        """Format device time from Unix timestamp to human-readable format."""
        if device_time is None:
            return None
        
        try:
            from datetime import datetime
            # Convert string timestamp to integer and then to datetime
            timestamp = int(device_time)
            dt = datetime.fromtimestamp(timestamp)
            return dt.strftime("%Y-%m-%d %H:%M:%S")
        except (ValueError, OSError) as ex:
            # If parsing fails, return the original value
            _LOGGER.warning("Failed to format device time '%s': %s", device_time, ex)
            return device_time
